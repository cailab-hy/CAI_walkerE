...
@configclass
class MoveBoxRewardCfg:
    # Move box keeps the base stable but prioritizes upper-body motion tracking via AMP.
    ...
    body_orientation_l2 = RewTerm(
        func=mdp.body_orientation_l2, params={"asset_cfg": SceneEntityCfg("robot", body_names="pelvis")}, weight=-0.6
    )
    ...
    joint_deviation_body = RewTerm(
        func=mdp.joint_deviation_l1,
        weight=-0.15,
        params={"asset_cfg": SceneEntityCfg("robot", joint_names=["body_yaw_joint"])},
    )
...
@configclass
class TienKungMoveBoxFlatEnvCfg:
...
@configclass
class TienKungMoveBoxAgentCfg(RslRlOnPolicyRunnerCfg):
    seed = 42
    device = "cuda:0"
    ...
    experiment_name = "movebox"
    run_name = ""
    logger = "tensorboard"
    neptune_project = "movebox"
    wandb_project = "movebox"
    resume = False
    load_run = ".*"
