...
class TienKungEnv(VecEnv):
    def __init__(
        ...
        self.motion_dof = int((self.amp_loader_display.trajectories_full[0].shape[1] - 12) / 2)
        self.amp_full_dof = getattr(self.cfg, "amp_full_dof", self.motion_dof > 20)
    ...
    def init_buffers(self):
        ...
        self.left_arm_full_ids, _ = self.robot.find_joints(
            name_keys=[
                "shoulder_pitch_l_joint",
                "shoulder_roll_l_joint",
                "shoulder_yaw_l_joint",
        ...
        self.right_arm_full_ids, _ = self.robot.find_joints(
            name_keys=[
                "shoulder_pitch_r_joint",
                "shoulder_roll_r_joint",
                "shoulder_yaw_r_joint",
        ...
        self.torso_head_ids, _ = self.robot.find_joints(
            name_keys=["body_yaw_joint", "head_yaw_joint", "head_pitch_joint", "head_roll_joint"],
            preserve_order=True,
        )
    ...
    def visualize_motion(self, time):
        ...
        dof_pos_frame = visual_motion_frame[6 : 6 + self.motion_dof]
        dof_vel_start = 12 + self.motion_dof
        dof_vel_frame = visual_motion_frame[dof_vel_start : dof_vel_start + self.motion_dof]

        if self.motion_dof == 20:
            dof_pos[:, self.left_leg_ids] = dof_pos_frame[0:6]
            dof_pos[:, self.right_leg_ids] = dof_pos_frame[6:12]
            dof_pos[:, self.left_arm_ids] = dof_pos_frame[12:16]
            dof_pos[:, self.right_arm_ids] = dof_pos_frame[16:20]

            dof_vel[:, self.left_leg_ids] = dof_vel_frame[0:6]
            dof_vel[:, self.right_leg_ids] = dof_vel_frame[6:12]
            dof_vel[:, self.left_arm_ids] = dof_vel_frame[12:16]
            dof_vel[:, self.right_arm_ids] = dof_vel_frame[16:20]
        elif self.motion_dof == 24:
            dof_pos[:, self.left_leg_ids] = dof_pos_frame[0:6]
            dof_pos[:, self.right_leg_ids] = dof_pos_frame[6:12]
            dof_pos[:, self.torso_head_ids] = dof_pos_frame[12:16]
            dof_pos[:, self.left_arm_ids] = dof_pos_frame[16:20]
            dof_pos[:, self.right_arm_ids] = dof_pos_frame[20:24]

            dof_vel[:, self.left_leg_ids] = dof_vel_frame[0:6]
            dof_vel[:, self.right_leg_ids] = dof_vel_frame[6:12]
            dof_vel[:, self.torso_head_ids] = dof_vel_frame[12:16]
            dof_vel[:, self.left_arm_ids] = dof_vel_frame[16:20]
            dof_vel[:, self.right_arm_ids] = dof_vel_frame[20:24]
        elif self.motion_dof == 30:
            dof_pos[:, self.left_leg_ids] = dof_pos_frame[0:6]
            dof_pos[:, self.right_leg_ids] = dof_pos_frame[6:12]
            dof_pos[:, self.torso_head_ids] = dof_pos_frame[12:16]
            dof_pos[:, self.left_arm_full_ids] = dof_pos_frame[16:23]
            dof_pos[:, self.right_arm_full_ids] = dof_pos_frame[23:30]

            dof_vel[:, self.left_leg_ids] = dof_vel_frame[0:6]
            dof_vel[:, self.right_leg_ids] = dof_vel_frame[6:12]
            dof_vel[:, self.torso_head_ids] = dof_vel_frame[12:16]
            dof_vel[:, self.left_arm_full_ids] = dof_vel_frame[16:23]
            dof_vel[:, self.right_arm_full_ids] = dof_vel_frame[23:30]
        else:
            raise ValueError(f"Unsupported motion DOF count: {self.motion_dof}")
        ...
        if self.motion_dof == 30:
            self.left_arm_dof_pos = dof_pos[:, self.left_arm_full_ids]
            self.right_arm_dof_pos = dof_pos[:, self.right_arm_full_ids]
            self.left_arm_dof_vel = dof_vel[:, self.left_arm_full_ids]
            self.right_arm_dof_vel = dof_vel[:, self.right_arm_full_ids]
            torso_head_pos = dof_pos[:, self.torso_head_ids]
            torso_head_vel = dof_vel[:, self.torso_head_ids]
            return torch.cat(
                (
    ...
    def get_amp_obs_for_expert_trans(self):
        ...
        if self.motion_dof == 30:
            self.left_arm_dof_pos = self.robot.data.joint_pos[:, self.left_arm_full_ids]
            self.right_arm_dof_pos = self.robot.data.joint_pos[:, self.right_arm_full_ids]
            self.left_arm_dof_vel = self.robot.data.joint_vel[:, self.left_arm_full_ids]
            self.right_arm_dof_vel = self.robot.data.joint_vel[:, self.right_arm_full_ids]
            torso_head_pos = self.robot.data.joint_pos[:, self.torso_head_ids]
            torso_head_vel = self.robot.data.joint_vel[:, self.torso_head_ids]
            return torch.cat(
                (
                    self.right_arm_dof_pos,
                    self.left_arm_dof_pos,
